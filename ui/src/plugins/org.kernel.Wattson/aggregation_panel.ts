...
export class WattsonAggregationPanel extends AggregationPanel
  ...
  view({attrs}: m.CVnode<AggregationPanelAttrs>) {
    return m(
      ...
      m(
        ...
        m('.button.pf-button--filled',
          {
            onclick: () => {
              // Toggle between µW and mW
              this.scaleNumericData = !this.scaleNumericData;
            },
            type: 'button',
          ...
          this.scaleNumericData ? 'Current scale: µW' : 'Current scale: mW',
        ),
          ...
          m(
            ...
            attrs.data.columnSums.map((sum) => {
              ...
              if (sum === '') {
                ...
                displaySum = String(this.scaleNumericData ?
                                    parseFloat(sum) * 1000 : sum);
              }
    ...
    switch (column.kind) {
      ...
      default:
        if (typeof value === 'number') {
          return (
            this.scaleNumericData && column.title.includes("estimated mW")
              ? value * 1000 : value
          );
        }
        return value;
